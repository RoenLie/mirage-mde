...
let anchorToExternalRegex = new RegExp(/(<a.*?https?:\/\/.*?[^a]>)+?/g);
...
function addAnchorTargetBlank(htmlText) {
	...
	while ((match = anchorToExternalRegex.exec(htmlText)) !== null) {
		...
		let linkString = match[0];

		if (linkString.indexOf('target=') === -1) {
			let fixedLinkString = linkString.replace(/>$/, ' target="_blank">');
...
function removeListStyleWhenCheckbox(htmlText) {
	...
	let listItems = htmlDoc.getElementsByTagName('li');

	for (let i = 0; i < listItems.length; i++) {
		let listItem = listItems[i];

		for (let j = 0; j < listItem.children.length; j++) {
			let listItemChild = listItem.children[j];
...
function createToolbarDropdown(options, enableTooltips, shortcuts, parent) {
	let el = createToolbarButton(options, false, enableTooltips, shortcuts, 'button', parent);
	...
	for (let childrenIndex = 0; childrenIndex < options.children.length; childrenIndex++) {
		let child = options.children[childrenIndex];
		var childElement;
...
function createToolbarButton(options, enableActions, enableTooltips, shortcuts, markup, parent) {
	options = options || {};
	let el = document.createElement(markup);
	...
	let classNamePrefix = parent.options.toolbarButtonClassPrefix ? parent.options.toolbarButtonClassPrefix + '-' : '';
	...
	for (let classNameIndex = 0; classNameIndex < classNameParts.length; classNameIndex++) {
		let classNamePart = classNameParts[classNameIndex];
	...
	if (iconClasses.length > 0) {
		...
		for (let iconClassIndex = 0; iconClassIndex < iconClasses.length; iconClassIndex++) {
			let iconClass = iconClasses[iconClassIndex];
...
function toggleFullScreen(editor) {
	...
	let sidebyside = wrapper.nextSibling;
	...
	if (sidebyside.classList.contains('editor-preview-active-side')) {
		if (editor.options.sideBySideFullscreen === false) {
			...
			let easyMDEContainer = wrapper.parentNode;
			if (cm.getOption('fullScreen'))
...
function drawLink(editor) {
	...
	if (options.promptURLs) {
		let result = prompt(options.promptTexts.link, url);
		if (!result)
			return false;
...
function drawImage(editor) {
	...
	let url = 'https://';
	if (options.promptURLs) {
		let result = prompt(options.promptTexts.image, url);
		if (!result)
			return false;
...
function afterImageUploaded(editor, url) {
	...
	let ext = imageName.substring(imageName.lastIndexOf('.') + 1).replace(/\?.*$/, '').toLowerCase();
	...
	if ([ 'png', 'jpg', 'jpeg', 'gif', 'svg', 'apng', 'avif', 'webp' ].includes(ext)) {
		...
		let text_link = options.insertTexts.link;
...
function toggleSideBySide(editor) {
	...
	let toolbarButton = editor.toolbarElements && editor.toolbarElements['side-by-side'];
	...
	if (previewNormal.classList.contains('editor-preview-active')) {
		...
		let toolbar_div = editor.toolbar_div;
		toolbar.classList.remove('active');
		toolbar_div.classList.remove('disabled-for-preview');
	}

	let sideBySideRenderingFunction = function() {
		let newValue = editor.options.previewRender(editor.value(), preview);
		if (newValue != null)
...
function togglePreview(editor) {
	...
	let preview_result = editor.options.previewRender(editor.value(), preview);
...
function _toggleLine(cm, name, liststyle) {
	...
	let listRegexp = /^(\s*)(\*|-|\+|\d*\.)(\s+)/;
	let whitespacesRegexp = /^\s*/;
	...
	let _checkChar = function(name, char) {
		let map = {
			'quote':          '>',
			'unordered-list': '\\' + liststyle,
			'ordered-list':   '\\d+.',
		};
		let rt = new RegExp(map[name]);

		return char && rt.test(char);
	};

	let _toggle = function(name, text, untoggleOnly) {
		let arr = listRegexp.exec(text);
...
function wordCount(data) {
	let pattern = /[a-zA-Z0-9_\u00A0-\u02AF\u0392-\u03c9\u0410-\u04F9]+|[\u4E00-\u9FFF\u3400-\u4dbf\uf900-\ufaff\u3040-\u309f\uac00-\ud7af]+/g;
...
EasyMDE.prototype.updateStatusBar = function(itemName, content) {
	...
	let matchingClasses = this.gui.statusbar.getElementsByClassName(itemName);
...
EasyMDE.prototype.markdown = function(text) {
	if (marked) {
		...
		if (this.options && this.options.renderingConfig && this.options.renderingConfig.codeSyntaxHighlighting === true) {
			...
			let hljs = this.options.renderingConfig.hljs || window.hljs;
...
EasyMDE.prototype.render = function(el) {
	...
	let easyMDEContainer = document.createElement('div');
	...
	function calcHeight(naturalWidth, naturalHeight) {
		let height;
		let viewportWidth = window.getComputedStyle(document.querySelector('.CodeMirror-sizer')).width.replace('px', '');
	...
	function handleImages() {
		...
		easyMDEContainer.querySelectorAll('.cm-image-marker').forEach((e) => {
			...
			if (!parentEl.hasAttribute('data-img-src')) {
				let srcAttr = parentEl.innerText.match('\\((.*)\\)'); // might require better parsing according to markdown spec
				...
				if (srcAttr && srcAttr.length >= 2) {
					...
					if (options.imagesPreviewHandler) {
						let newSrc = options.imagesPreviewHandler(srcAttr[1]);
...
EasyMDE.prototype.autosave = function() {
	if (isLocalStorageAvailable()) {
		...
		if (el != null && el != undefined && el != '') {
			let d = new Date();
			let dd = new Intl.DateTimeFormat([ this.options.autosave.timeFormat.locale, 'en-US' ], this.options.autosave.timeFormat.format).format(d);
			let save = this.options.autosave.text == undefined ? 'Autosaved: ' : this.options.autosave.text;
...
EasyMDE.prototype.openBrowseFileWindow = function(onSuccess, onError) {
	let self = this;
	let imageInput = this.gui.toolbar.getElementsByClassName('imageInput')[0];
...
EasyMDE.prototype.uploadImageUsingCustomFunction = function(imageUploadFunction, file) {
	...
	function onError(errorMessage) {
		let filledErrorMessage = fillErrorMessage(errorMessage);
		// show error on status bar and reset after 10000ms
...
EasyMDE.prototype.setPreviewMaxHeight = function() {
	...
	let paddingTop = parseInt(window.getComputedStyle(wrapper).paddingTop);
	let borderTopWidth = parseInt(window.getComputedStyle(wrapper).borderTopWidth);
	let optionsMaxHeight = parseInt(this.options.maxHeight);
	let wrapperMaxHeight = optionsMaxHeight + paddingTop * 2 + borderTopWidth * 2;
	let previewMaxHeight = wrapperMaxHeight.toString() + 'px';
...
EasyMDE.prototype.createSideBySide = function() {
	...
	cm.on('scroll', (v) => {
		...
		let move = (preview.scrollHeight - preview.clientHeight) * ratio;
		...
		if (typeof status[i] === 'object') {
			...
			if (name === 'words') {
				...
				onActivity = function(el) {
					...
					let posLine = pos.line + 1;
					let posColumn = pos.ch + 1;
...
EasyMDE.prototype.value = function(val) {
	...
	if (val === undefined) {
		...
		if (this.isPreviewActive()) {
			let wrapper = cm.getWrapperElement();
			let preview = wrapper.lastChild;
			let preview_result = this.options.previewRender(val, preview);
			if (preview_result !== null)
				preview.innerHTML = preview_result;
		}
